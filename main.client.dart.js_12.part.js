((a,b)=>{a[b]=a[b]||{}})(self,"$__dart_deferred_initializers__")
$__dart_deferred_initializers__.current=function(a,b,c,$){var B,C,G,A={
bE(d){var x=$.ne.k(0,d)
if(x==null){x=new A.f5(d,B.f([],y.C))
$.ne.i(0,d,x)}return x},
f6:function f6(d,e){this.a=d
this.b=e},
dk:function dk(d,e){this.c=d
this.a=e},
hq:function hq(d,e,f,g,h,i,j){var _=this
_.d$=d
_.e$=e
_.f$=f
_.cy=null
_.db=g
_.c=_.b=_.a=null
_.d=h
_.e=null
_.f=i
_.w=_.r=null
_.x=j
_.Q=_.z=_.y=null
_.as=!1
_.at=!0
_.ax=!1
_.CW=null
_.cx=!1},
bb:function bb(d,e,f){var _=this
_.w=d
_.x=e
_.y=null
_.z=f
_.d=$
_.c=_.b=_.a=null},
f5:function f5(d,e){var _=this
_.a=d
_.e=_.d=_.c=_.b=$
_.f=e
_.r=!0},
ix:function ix(){},
i9:function i9(d,e,f,g,h){var _=this
_.c=d
_.d=e
_.e=f
_.at=g
_.a=h},
iP:function iP(d,e){this.a=d
this.b=e},
fA:function fA(d,e){this.c=d
this.a=e},
ha:function ha(d,e){this.c=d
this.a=e},
ep:function ep(d,e,f){this.c=d
this.d=e
this.a=f},
r4(){return new A.bt(null)},
bt:function bt(d){this.a=d},
hd:function hd(){this.d=!1
this.c=this.a=null},
k6:function k6(d){this.a=d},
k5:function k5(d){this.a=d},
qB(d){var x,w,v=y.w,u=B.P(v,v)
for(x=0;x<B.V(d.length);++x){w=B.y(d.item(x))
u.i(0,B.v(w.name),B.v(w.value))}return u}},E,H,I,F,D
B=c[0]
C=c[2]
G=c[11]
A=a.updateHolder(c[6],A)
E=c[17]
H=c[12]
I=c[13]
F=c[9]
D=c[10]
A.f6.prototype={
az(){return"AttachTarget."+this.b}}
A.dk.prototype={
aa(){var x=B.ca(y.b),w=($.W+1)%16777215
$.W=w
return new A.hq(null,!1,!1,x,w,this,C.f)}}
A.hq.prototype={
aM(){var x=this.f
x.toString
y.h.a(x)
return E.ar},
ah(){var x,w,v=this.f
v.toString
y.h.a(v)
x=this.e
x.toString
x=new A.bb(B.f([],y.k),E.t,x)
x.bB("")
w=A.bE(x.x)
C.b.m(w.f,x)
w.r=!0
x.se5(v.c)
return x},
ac(d){var x
y.n.a(d)
x=this.f
x.toString
y.h.a(x)
d.si0(E.t)
d.se5(x.c)},
ba(){var x,w
this.da()
x=this.d$
x.toString
y.n.a(x)
w=this.e
w.toString
x.shf(w)},
aB(){var x,w
this.eV()
x=this.d$
x.toString
y.n.a(x)
w=A.bE(x.x)
C.b.F(w.f,x)
w.aW()}}
A.bb.prototype={
si0(d){var x=this,w=x.x
if(w===d)return
w=A.bE(w)
C.b.F(w.f,x)
w.aW()
x.x=d
w=A.bE(d)
C.b.m(w.f,x)
w.r=!0
A.bE(x.x).aW()},
se5(d){y.f.a(d)
if(this.y===d)return
this.y=d
A.bE(this.x).aW()},
shf(d){if(this.z===d)return
this.z=d
A.bE(this.x).ex(!0)},
ap(d,e){var x,w,v,u,t=this
d.a=t
try{x=d.gL()
w=e==null?null:e.gL()
if(w==null&&C.b.I(t.w,x))return
if(w!=null&&!C.b.I(t.w,w))w=null
v=t.w
C.b.F(v,x)
u=w!=null?C.b.aj(v,w)+1:0
C.b.ee(v,u,x)
A.bE(t.x).aW()}finally{d.ai()}},
F(d,e){C.b.F(this.w,e.gL())
e.a=null
A.bE(this.x).aW()}}
A.f5.prototype={
ge9(){var x,w=this,v=w.b
if(v===$){x=B.y(B.j(b.G.document).querySelector(w.a.b))
x.toString
w.b!==$&&B.eY()
w.b=x
v=x}return v},
ex(d){var x,w,v,u,t,s,r,q,p,o,n,m=this
if(d||m.r){C.b.an(m.f,new A.ix())
m.r=!1}x=m.c
if(x===$){w=A.qB(B.j(m.ge9().attributes))
m.c!==$&&B.eY()
m.c=w
x=w}for(v=m.f,u=v.length,t=0;t<v.length;v.length===u||(0,B.ax)(v),++t){s=v[t].y
if(s!=null)x.S(0,s)}r=B.ms(y.w)
for(q=0;v=m.ge9(),q<B.V(B.j(v.attributes).length);++q)r.m(0,B.v(B.y(B.j(v.attributes).item(q)).name))
if(x.a!==0)for(u=new B.aB(x,B.h(x).h("aB<1,2>")).gA(0);u.n();){p=u.d
o=p.a
B.f7(v,o,p.b)
r.F(0,o)}if(r.a!==0)for(u=B.nX(r,r.r,r.$ti.c),o=u.$ti.c;u.n();){n=u.d
if(n==null)n=o.a(n)
v.removeAttribute(n)}},
aW(){return this.ex(!1)}}
A.i9.prototype={
N(d){var x=this,w=null,v=y.w
v=B.P(v,v)
v.i(0,"cx",x.c)
v.i(0,"cy",x.d)
v.i(0,"r",x.e)
return new B.af("circle",w,w,w,v,w,x.at,w)}}
A.iP.prototype={
az(){return"Display."+this.b}}
A.fA.prototype={
N(d){var x=y.F
return new A.ep(this.c,B.f([D.bD(B.f([],x),"M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z")],x),null)}}
A.ha.prototype={
N(d){var x=y.F
return new A.ep(this.c,B.f([new A.i9("12","12","4",B.f([],x),null),D.bD(B.f([],x),"M12 4h.01"),D.bD(B.f([],x),"M20 12h.01"),D.bD(B.f([],x),"M12 20h.01"),D.bD(B.f([],x),"M4 12h.01"),D.bD(B.f([],x),"M17.657 6.343h.01"),D.bD(B.f([],x),"M17.657 17.657h.01"),D.bD(B.f([],x),"M6.343 17.657h.01"),D.bD(B.f([],x),"M6.343 6.343h.01")],x),null)}}
A.ep.prototype={
N(d){var x=this.c,w=y.w
return D.pd(this.d,B.cc(["fill","none","stroke","currentColor","stroke-width","2","stroke-linecap","round","stroke-linejoin","round"],w,w),new F.aF("px",x),"0 0 24 24",new F.aF("px",x))}}
A.bt.prototype={
bM(){return new A.hd()}}
A.hd.prototype={
be(){this.ci()
this.d=B.b2(B.y(B.j(b.G.document).documentElement).getAttribute("data-theme"))==="dark"},
N(d){var x,w=this,v=null,u=y.F,t=B.f([],u),s=w.d?"dark":"light",r=y.w
t.push(new A.dk(B.cc(["data-theme",s],r,r),v))
r=B.cc(["aria-label","Theme Toggle"],r,r)
s=F.kT(w.d?E.y:v,v,v,v,v,v,v)
s=D.dg(B.f([new A.fA(20,v)],u),s)
x=F.kT(w.d?v:E.y,v,v,v,v,v,v)
t.push(G.oT(B.f([s,D.dg(B.f([new A.ha(20,v)],u),x)],u),r,"theme-toggle",new A.k6(w),v))
return new G.c9(t,v)}}
var z=a.updateTypes(["d(bb,bb)"])
A.ix.prototype={
$2(d,e){var x=y.n
x.a(d)
x.a(e)
return d.z-e.z},
$S:z+0}
A.k6.prototype={
$0(){var x,w=this.a
w.aK(new A.k5(w))
x=B.j(B.j(b.G.window).localStorage)
w=w.d?"dark":"light"
x.setItem("jaspr:theme",w)},
$S:0}
A.k5.prototype={
$0(){var x=this.a
x.d=!x.d},
$S:0};(function inheritance(){var x=a.inheritMany,w=a.inherit
x(B.co,[A.f6,A.iP])
w(A.dk,B.n)
w(A.hq,B.bO)
w(A.bb,B.dv)
w(A.f5,B.i)
w(A.ix,B.bG)
x(I.J,[A.i9,A.fA,A.ha,A.ep])
w(A.bt,H.at)
w(A.hd,H.ad)
x(B.bn,[A.k6,A.k5])})()
B.bm(b.typeUniverse,JSON.parse('{"bb":{"aq":[],"mw":[],"bP":[]},"dk":{"n":[]},"hq":{"ai":[],"l":[],"a1":[]},"i9":{"J":[],"n":[]},"fA":{"J":[],"n":[]},"ha":{"J":[],"n":[]},"ep":{"J":[],"n":[]},"bt":{"at":[],"n":[]},"hd":{"ad":["bt"],"ad.T":"bt"}}'))
var y={h:B.t("dk"),n:B.t("bb"),b:B.t("l"),C:B.t("r<bb>"),F:B.t("r<n>"),k:B.t("r<p>"),w:B.t("c"),f:B.t("w<c,c>?")};(function constants(){var x=a.makeConstList
E.t=new A.f6(0,"html")
E.y=new A.iP(0,"none")
E.ar=x([],y.F)})();(function staticFields(){$.ne=B.P(B.t("f6"),B.t("f5"))})()};
(a=>{a["s+2n3NZKCuBu+a/9k/E+aqzm1E0="]=a.current})($__dart_deferred_initializers__);
//# sourceMappingURL=main.client.dart.js_12.part.js.map
